...
public partial class Finalstory : Control
{
	// ── Enemy HUD ───────────────────────────────────────────────────────────
	...
	private void ApplyBattleStats()
	{
		...
		_battleAtk  = _battleStr * 200000;   // สูตรเดิม Atk = Str*2
		_battleMAtk = _battleInt * 500000;   // สูตรเดิม MAtk = Int*6
			int battleMaxHp = 50 + (_battleStr * 200000);  // สูตรเดียวกับ CreateWarrior
	...
	private void OnBattleWin()
	{
		...
		temp.Save();
		ShowResultButtons();
		GetTree().ChangeSceneToFile("res://ending2.tscn");
	}
